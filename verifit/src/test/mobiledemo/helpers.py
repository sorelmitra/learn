...
IOS_BASE_CAPS = {
	'app': os.path.abspath('/Users/sorel/1data/w/github/other/appium/sample-code/apps/TestApp.app.zip'),
...
if os.getenv('SAUCE_LABS') and os.getenv('SAUCE_USERNAME') and os.getenv('SAUCE_ACCESS_KEY'):
	...
	class DummyJobs:
		def update_job(self, session_id, *args, **kwargs):
			print(f"Sauce is not configured. Session {session_id}: {args} {kwargs}")
	class DummySauce:
		jobs = DummyJobs()
	sauce = DummySauce()
