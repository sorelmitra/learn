...
for k in range(0, tide_cycle_length):
	...
	hw = TideHeight(
		time=a_date.time(),
		height=compute_current_height(6),
		life_cycle=TideHeight.HW
	)
	a_date = a_date + datetime.timedelta(hours=6)
	lw = TideHeight(
		time=a_date.time(),
		height=compute_current_height(0),
...
current_day = random.randint(0, len(tide_days) - 1)
current_tide = tide_days[current_day]
