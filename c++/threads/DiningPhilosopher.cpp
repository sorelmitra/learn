...
unsigned int DiningPhilosopher::count=0;

std::string DiningPhilosopher::toString() {
    std::ostringstream ss;
    ss << "phil " << number;
    return ss.str();
}

void DiningPhilosopher::logMsg(std::string s) {
    pthread_mutex_lock(&mutex);
    cerr << toString() << " " << s << endl;
    pthread_mutex_unlock(&mutex);
}

void DiningPhilosopher::think() {
    logMsg("thinking");
    usleep(random() % 1000);
}

void DiningPhilosopher::eat() {
    stringstream ss;
...
int main(int argc, char **argv) {
    ...
    for (int i=0; i<n; i++) {
	   ...
	   r=i-1;
        ...
        phils[i]=new DiningPhilosopher(&chops, l, r, syncThread);
        phils[i]->start();
    }
    for (int i=0; i<n; i++) {
        phils[i]->join();
